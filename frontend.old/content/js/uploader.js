const uploadFiles = (() => {
    ...
    const ENDPOINTS = {
        UPLOAD: 'https://localhost:49163/api/files/upload',
        UPLOAD_STATUS: 'https://localhost:49163/api/files/upload-status',
        UPLOAD_REQUEST: 'https://localhost:49163/api/files/upload-request'
    }
    ...
    const uploadFile = (file, options) => {
        debugger;
        res = {fileId: uniqueAlphaNumericId()};
        options = {...options, ...res};
...
const uploadAndTrackFiles = (() => {
    const files = new Map();
    const progressBox = document.getElementById("form-upload");
    const FILE_STATUS = {
        PENDING: 'pending',
        UPLOADING: 'uploading',
        PAUSED: 'paused',
    ...
    const onError = (e, file) => {
        const fileObj = file ? files.get(file) : files.get(e.file);
